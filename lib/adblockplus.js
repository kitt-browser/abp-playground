...
require.scopes["downloader"] = (function()
{
  ...
  Downloader.prototype = {
    ...
    _download: function(downloadable, redirects)
    {
      ...
      try
      {
        console.log('Downloader XHR', downloadUrl);
      ...
      {
        console.log('Downloader XHR loading...');
        if (onShutdown.done)
        {
          return;
        }
...
require.scopes["filterClasses"] = (function()
{
  ...
  ActiveFilter.prototype = {
    ...
    get domains()
    {
      ...
      var d = Object.getOwnPropertyDescriptor(this, "domains");
      if (!d || d.writable) {
  ...
  RegExpFilter.prototype = {
    ...
    get regexp()
    {
      if (!this.regexpSource) {return null;}
    ...
    {
      if (this.regexp && this.regexp.test(location) && (RegExpFilter.typeMap[contentType] & this.contentType) != 0 && (this.thirdParty == null || this.thirdParty == thirdParty) && this.isActiveOnDomain(docDomain))
      {
        return true;
      }
      return false;
...
require.scopes["filterStorage"] = (function()
{
  ...
  var FilterStorage = exports.FilterStorage = {
    ...
    get sourceFile()
    {
      ...
      console.log('define getter for sourceFile');
    ...
    {
      console.log('loadFromDisk', this._loading)
      if (this._loading)
      {
        return;
      }
      ...
      var doneReading = function(parser)
      {
        ...
        console.log('  >>> adblockplus doneReading', parser.linesProcessed);
        var specialMap = {
          "~il~": true,
          "~wl~": true,
          "~fl~": true,
        ...
        {
          console.log('  >>> parser subscription[', i, '] length:', parser.subscriptions[i].filters.length);
      ...
      else
      {
        ...
        {
          console.log('  >> callback e', e, statData, sourceFile);
          if (e || !statData.exists)
          {
            this.firstRun = true;
            this._loading = false;
  ...
  INIParser.prototype = {
    ...
    {
      //console.log('  >>processing (INI PARSER)', val);
...
require.scopes["matcher"] = (function()
{
  ...
  Matcher.prototype = {
    ...
    _checkEntryMatch: function(keyword, location, contentType, docDomain, thirdParty)
    {
      ...
      {
        var filter = list[i];
        //console.log(' >>> filter >>> _checkEntryMatch', filter.text);
  ...
  CombinedMatcher.prototype = {
    ...
    add: function(filter)
    {
      //console.log(' >>> CombinedMatcher add');
      ...
      {
        //console.log('adding to blacklist', Object.keys(this.blacklist.filterByKeyword).length)
    ...
    matchesAnyInternal: function(location, contentType, docDomain, thirdParty)
    {
      ...
      candidates.push("");
      //console.log('CombinedMatcher, matchesAnyInternal, candidates', candidates);
      var blacklistHit = null;
      //console.log('whitelist length', Object.keys(this.whitelist.filterByKeyword).length);
      //console.log('blacklist length', Object.keys(this.blacklist.filterByKeyword).length);
      for (var i = 0, l = candidates.length; i < l; i++)
      {
        var substr = candidates[i];
    ...
    matchesAny: function(location, contentType, docDomain, thirdParty)
    {
      ...
      //console.log('CombinedMatcher key', key)
      if (key in this.resultCache)
      {
        return this.resultCache[key];
      }
      //console.log('CombinedMatcher matchesAnyInternal before');
      var result = this.matchesAnyInternal(location, contentType, docDomain, thirdParty);
      //console.log('CombinedMatcher matchesAnyInternal result', result);
...
require.scopes["filterListener"] = (function()
{
  ...
  function onGenericChange(action)
  {
    if (action == "load")
    {
      ...
      ElemHide.clear();
      //console.log('  >> onGenericChange', FilterStorage.subscriptions.length);
      for (var _loopIndex24 = 0; _loopIndex24 < FilterStorage.subscriptions.length; ++_loopIndex24)
      {
        ...
        //console.log('  >onGenericChange loop', _loopIndex24, 'length', subscription.filters.length);
...
require.scopes["synchronizer"] = (function()
{
  ...
  var Synchronizer = exports.Synchronizer = {
    ...
    {
      console.log('Downloader execute', this._getDownloadable(subscription, manual).url);
...
require.scopes["whitelisting"] = (function()
{
  ...
  {
    //console.log('isWhitelisted', url, parentUrl, type);
    // KITTHACK: url should be defined. I've added the check.
    //if ( ! url) {return false}
  ...
  {
    //console.log('isFrameWhitelisted', page, frame, type);
    ...
    for (; frame != null; frame = frame.parent)
    {
      ...
      //console.log('frame2', frame, frame.url);
      //console.log('before isWhitelisted', frame.url, (frame.parent || {}).url, type);
